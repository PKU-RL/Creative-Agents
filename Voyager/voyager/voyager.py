...
class Voyager:
    ...
    def reset(self, task, context="", reset_env=True, init_inventory={}):
    ...
    def step(self, init_message=None):
        if self.action_agent_rollout_num_iter < 0:
            raise ValueError("Agent must be reset before stepping")

        ai_message = AIMessage(content=init_message)
    ...
    def rollout(self, *, task, context, reset_env=True, init_inventory={}, init_message=None):
        self.reset(task=task, context=context, reset_env=reset_env, init_inventory=init_inventory)
        while True:
            messages, reward, done, info = self.step(init_message=init_message)
            if done:
                break
        return messages, reward, done, info

    def learn(self, reset_env=True, init_inventory={}, init_message=None):
        if self.resume:
            # keep the inventory
            self.env.reset(
                options={
