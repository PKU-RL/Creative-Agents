...
def test_net(cfg,
    ...
    for sample_idx, (rendering_images, ground_truth_volume) in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            ground_truth_volume = ground_truth_volume.to(torch.float32)
            ...
            if output_dir and sample_idx < 3:
                ...
                # rendering_images[:,:,[0,1,2]] = rendering_images[:,:,[2,1,0]]
                # print(np.where(rendering_images!=0))
                # print(rendering_images.shape)
                input_image = Image.fromarray(rendering_images)
                input_image = input_image.convert('RGB')
                # print(input_image)
                input_path = os.path.join(img_dir, 'test')
                if not os.path.exists(input_path):
                    os.mkdir(input_path)
                input_image.save(os.path.join(input_path,'input_image_{}.png'.format(sample_idx)))
                ...
                rendering_views = utils.binvox_visualization.get_volume_views(gtv, os.path.join(img_dir, 'test'),
                                                                              epoch_idx,ground_truth= True)
                # test_writer.add_image('Test Sample#%02d/Volume GroundTruth' % sample_idx, rendering_views, epoch_idx)

            # Print sample loss and IoU
            print('[INFO] %s Test[%d/%d] EDLoss = %.4f RLoss = %.4f IoU = %s' %
                  (dt.now(), sample_idx + 1, n_samples, encoder_loss.item(),
